...
class Trainer(object):
    ...
    def train(self):
        ...
        if getattr(self.args, "skip_train", False):
            self.logger.info("Skip training enabled: saving initial checkpoint and evaluating directly.")
            os.makedirs(self.args.output_dir, exist_ok=True)
            self.stopper.save_checkpoint(
                val_loss=0.0,
                model=model_to_save,

...
class SeqTrainer(Trainer):
    ...
    def _create_model(self):
        '''create your model'''
        if self.args.emb_argu:
            if self.args.model_name == 'sasrec_seq':
                self.model = SASRecPLUS(self.user_num, self.item_num, self.device, self.args)
    ...
    def save_user_emb(self):
        ...
        output_path = os.path.join(self.args.dataset_dir, "usr_emb_{}.pkl".format(self.args.model_name))
        pickle.dump(user_emb, open(output_path, "wb"))
    ...
    def save_item_emb(self):
        ...
        output_path = os.path.join(self.args.dataset_dir, "itm_emb_{}.pkl".format(self.args.model_name))
    ...
    def test_group(self):
        ...
        # res_len_dict = metric_len_report(pred_rank.detach().cpu().numpy(), seq_len.detach().cpu().numpy(), aug_len=self.args.aug_seq_len, args=self.args)
        # res_pop_dict = metric_pop_report(pred_rank.detach().cpu().numpy(), self.item_pop, target_items.detach().cpu().numpy(), args=self.args)
        # hr_len, ndcg_len, count_len = metric_len_5group(pred_rank.detach().cpu().numpy(), seq_len.detach().cpu().numpy(), [4, 8, 12, 16])
        hr_pop, ndcg_pop, count_pop = metric_pop_5group(pred_rank.detach().cpu().numpy(), self.item_pop,  target_items.detach().cpu().numpy(), [5,7,13,36])

...
class SASRecBackbone(nn.Module):
    ...
    def forward(self, seqs, log_seqs):
        ...
        for i in range(len(self.attention_layers)):
            seqs = torch.transpose(seqs, 0, 1) # (U, T, C) -> (T, U, C)
            Q = self.attention_layernorms[i](seqs)
            mha_outputs, _ = self.attention_layers[i](Q, seqs, seqs,)
                                                    #   attn_mask=attention_mask)
            seqs = Q + mha_outputs # residual connection
            seqs = torch.transpose(seqs, 0, 1) # (T, U, C) -> (U, T, C)
...
class SASRec(BaseSeqModel):
    ...
    def predict(self,
        ...
        logits = item_embs.matmul(final_feat.unsqueeze(-1)).squeeze(-1) # (U, I, C) x (U, C, 1) -> (U, I, 1) -> (U, I)
